...
shelter1 = Shelter(name="Oakland Animal Services", address="1101 29th Ave", city="Oakland", state="California",
                   zipCode="94601", website="oaklandanimalservices.org", maximum_capacity=100)
...
shelter5 = Shelter(name="Palo Alto Humane Society", address="1149 Chestnut St.", city="Menlo Park", state="California",
                   zipCode="94025", website="paloaltohumane.org", maximum_capacity=100)
...
female_names = ['Bella', 'Lucy', 'Molly', 'Daisy', 'Maggie', 'Sophie', 'Sadie', 'Chloe', 'Bailey', 'Lola', 'Zoe',
...
for i, x in enumerate(male_names):
    profile = Profile()
    profile.picture = random.choice(puppy_images)
    new_puppy = Puppy(name=x, gender="male", dateOfBirth=CreateRandomAge(),
                      shelter_id=randint(1, 5), weight=CreateRandomWeight())
    new_puppy.profile = profile
...
for i, x in enumerate(female_names):
    profile = Profile()
    profile.picture = random.choice(puppy_images)
    new_puppy = Puppy(name=x, gender="female", dateOfBirth=CreateRandomAge(),
                      shelter_id=randint(1, 5), weight=CreateRandomWeight())
    new_puppy.profile = profile
    session.add(new_puppy)
    session.commit()
